...
def getDeviceCMD(devid):
    ...
    equ_index =getDeviceInfo(devid, 'equipment_index')  # 取出配對設備的索引值
...
def statisticsUsagePw(wh, devid):
    ...
    if wh > prev_wh:
        # 進行計算
        total = query_data['addwh'] + calculateWh(wh, devid)
        newvalues = {"$set": {"addwh":round(total)}}
        mycol.update_one(myquery, newvalues)
...
def process(json_data):
    ...
    if checkDeviceIDreg(request_data['id'], "register") != 0 and len(keylist) == 1: # 傳送Command。每 5秒檢查一次
        ...
        if countCMD(request_data['id'])  > 0:
            ...
            return {"status":"ok"}
    elif keylist.count('pw1') != 0 and checkDeviceIDreg(request_data['id'], "register") != 0: # 傳送設備 Power Meter。每 15秒傳送一次
